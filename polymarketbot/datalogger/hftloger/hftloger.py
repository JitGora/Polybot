        ...
        while True:
            ...
            try:
                async with websockets.connect("wss://ws-subscriptions-clob.polymarket.com/ws/market") as ws:
                    ...
                    # Subscribe to UP token (separate subscription)
                    subscribe_up = {
                        "type": "market",
                        "assets_ids": [self.current_event.up_token_id]
                    }

                    # Subscribe to DOWN token (separate subscription)
                    subscribe_down = {
                        "type": "market",
                        "assets_ids": [self.current_event.down_token_id]
                    }

                    # Send both subscriptions immediately (no delay for HFT)
                    await ws.send(json.dumps(subscribe_up))
                    await ws.send(json.dumps(subscribe_down))

                    print(f"\n✅ CLOB subscribed to UP token: {self.current_event.up_token_id[:16]}...")
                    print(f"✅ CLOB subscribed to DOWN token: {self.current_event.down_token_id[:16]}...")
